...
import (
	"errors"
	pkg2_v3 "github.com/hashicorp/go-licensing/v3"
...
func (x *LicenseGetResponse) codecDecodeSelfFromArray(l int, d *codec1978.Decoder) {
	...
	if r.TryDecodeAsNil() {
		...
		if false {
		} else if yyxt14 := z.Extension(z.I2Rtid(x.NomadLicense)); yyxt14 != nil {
			z.DecExtension(x.NomadLicense, yyxt14)
		} else {
			z.DecFallback(x.NomadLicense, false)
		}
	}
	...
	if r.TryDecodeAsNil() {
		...
		if false {
		} else if yyxt18 := z.Extension(z.I2Rtid(x.LastContact)); yyxt18 != nil {
			z.DecExtension(x.LastContact, yyxt18)
		} else {
			x.LastContact = (time.Duration)(r.DecodeInt64())
		}
	}
		...
		LABEL6:
			...
			if yyr2 || yy2arr2 {
				...
				if false {
					...
					r.EncodeInt(int64(x.VariablesLimit))
				}
			} else {
				r.WriteMapElemKey()
				if z.IsJSONHandle() {
					z.WriteStr("\"VariablesLimit\"")
				} else {
					r.EncodeStringEnc(codecSelferCcUTF8102, `VariablesLimit`)
				}
				r.WriteMapElemValue()
				if false {
				} else {
	...
	for yyj3 := 0; ; yyj3++ {
		...
		case "VariablesLimit":
			if r.TryDecodeAsNil() {
				...
				x.VariablesLimit = (int)(z.C.IntV(r.DecodeInt64(), codecSelferBitsize102))
			}
		case "Hash":
			if r.TryDecodeAsNil() {
				x.Hash = nil
...
func (x *QuotaLimit) codecDecodeSelfFromArray(l int, d *codec1978.Decoder) {
	...
	if r.TryDecodeAsNil() {
		...
		if false {
		} else if yyxt13 := z.Extension(z.I2Rtid(x.RegionLimit)); yyxt13 != nil {
			z.DecExtension(x.RegionLimit, yyxt13)
		} else {
			z.DecFallback(x.RegionLimit, false)
		}
	}
