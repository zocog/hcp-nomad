...
// expressions that are import accesses and rewrites them to use ImportExpr.
func rewriteImportSelector() func(ast.Node) ast.Node {
	...
	f = func(n ast.Expr) ast.Expr {
		...
		if cx, ok := n.(*ast.CallExpr); ok {
			...
			if ix, ok := f(cx.Fun).(*ImportExpr); ok {
		...
		case *ast.IndexExpr:
			// Traverse both the expression and the index
		...
		case *ast.SelectorExpr:
			...
			if ix, ok := fx.(*ImportExpr); ok {
				...
				ix.Keys = append(ix.Keys, ImportKey{Key: x.Sel.Name}) // Add root expression ident as key

				return ix
			}

			// we re-assign the expression regardless, as a nested
