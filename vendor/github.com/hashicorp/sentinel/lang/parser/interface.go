...
// ParseStmtFrom is a convenience function for parsing a statement.
// The arguments have the same meaning as for Parse, but the source must
// be a valid policy statement.
//
func ParseStmtFrom(fset *token.FileSet, filename string, src interface{}, mode Mode) (ast.Stmt, error) {
	// get source
	text, err := readSource(filename, src)
	if err != nil {
		return nil, err
...
func ParseStmt(x string) (ast.Stmt, error) {
	return ParseStmtFrom(token.NewFileSet(), "", []byte(x), 0)
}
